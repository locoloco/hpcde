! function() {
    ...
    viz.bP = function() {
        ...
        bP.width = function(_) {
            if (!arguments.length) return typeof width !== "undefined" ? width : 200;
            width = _;
            return bP;
        }
        bP.height = function(_) {
            if (!arguments.length) return typeof height !== "undefined" ? height : 200;
            height = _;
            return bP;
        }
                    ...
                    .attr("transform", function(d) {
                        if ((d.angle > Math.PI / 2 && d.angle < Math.PI) || (d.angle > (Math.PI + Math.PI / 2) && d.angle < Math.PI * 2)) {
                            return "rotate(" + (d.angle * 180 / Math.PI) + ")" +
                                "translate(" + (80 + 27) + ")" + (d.angle > Math.PI ? "rotate(0)" : "rotate(-180)");;
                        } else {
                            return "rotate(" + (d.angle * 180 / Math.PI) + ")" +
                                "translate(" + (80 + 27) + ")" +
